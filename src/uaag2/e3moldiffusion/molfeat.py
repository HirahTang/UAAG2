...
def bond_to_feature_vector(bond):
    ...
    :return: list
    """
    bond_feature = [safe_index(allowable_features["possible_bond_type_list"], str(bond.GetBondType()))]
    return bond_feature
...
def bond_feature_vector_to_dict(bond_feature):
    ...
    feature_dict = {"bond_type": allowable_features["possible_bond_type_list"][bond_type_idx]}
...
class AtomEncoder(nn.Module):
    def __init__(self, emb_dim, max_norm: float = 10.0, use_all_atom_features: bool = False):
...
if __name__ == "__main__":
    ...
    radius_edge_attr = torch.full(
        (radius_edge_index.size(1),), fill_value=radius_feat, device=pos.device, dtype=torch.long
    )
    ...
    combined_edge_index, combined_edge_attr = coalesce(
        index=combined_edge_index, value=combined_edge_attr, m=pos.size(0), n=pos.size(0), op="min"
    )
