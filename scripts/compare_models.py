...
def plot_proteingym(models, baseline_df, output_path):
    """models: list of dicts {label, summary(DataFrame benchmark/mean_spearman/std_spearman),
    color, marker, size}. Assays sorted by the FIRST model. Canonical style + all baselines;
    every model shares each assay's vertical line (no offset) with 5-iter error bars."""
    maps = [(m['label'], m['summary'].set_index('benchmark')[['mean_spearman', 'std_spearman']], m)
            for m in models]
    ref_map = maps[0][1]
    all_benchmarks = sorted(
        set().union(*[set(mp.index) for _, mp, _ in maps]),
        key=lambda b: ref_map.loc[b, 'mean_spearman'] if b in ref_map.index else -999,
        reverse=True,
    )
    ...
    # N models — all share the assay's vertical line (no horizontal offset), with error bars.
    for label, df_map, m in maps:
        y_vals = [float(df_map.loc[b, 'mean_spearman']) if b in df_map.index else np.nan
                  for b in all_benchmarks]
        y_err = [float(df_map.loc[b, 'std_spearman']) if b in df_map.index else 0.0
                 for b in all_benchmarks]
        ax.errorbar(x_pos, y_vals, yerr=y_err,
                    fmt=m['marker'], markersize=m['size'], color=m['color'],
...
# Display names for the two compared models; set in main() from --ctmc-label/--ddpm-label
# (or the results-dir basename). Keeps internal keys 'prior_ctmc'/'prior_ddpm' stable.
LABEL_MAP = {"prior_ctmc": "prior_ctmc", "prior_ddpm": "prior_ddpm"}
...
# Palette + marker cycle for the compared models (baselines keep their fixed colors above).
MODEL_PALETTE = ["#FFD700", "#00CED1", "#E76F51", "#9B5DE5", "#2A9D8F", "#264653", "#F72585"]
MODEL_MARKERS = ["*", "o", "s", "D", "^", "v", "P"]
MODEL_LABELS = []   # set in main(): the labels of the compared models (for show_err / colors)


def _build_barplot_data(models_uaa, baselines_dict):
    """models_uaa: list of (label, cp2(mean,std), puma(mean,std)). Plus the fixed baselines."""
    rows = []
    for label, cp2, puma in models_uaa:
        rows.append({"benchmark": "CP2",  "model": label, "spearmanr": cp2[0],  "std": cp2[1]})
        rows.append({"benchmark": "PUMA", "model": label, "spearmanr": puma[0], "std": puma[1]})
...
def _bar_chart(df, title, output_path):
    ...
    for i, model in enumerate(models):
        ...
        show_err = model in MODEL_LABELS
...
def _parse_model_spec(spec):
    """'label:results_dir:cp2_glob:puma_glob' -> (label, results_dir, cp2_glob, puma_glob).
    cp2_glob/puma_glob may be empty (model with no CP2/PUMA samples)."""
    parts = spec.split(":")
    if len(parts) < 2:
        raise SystemExit(f"--model needs at least 'label:results_dir' : got {spec!r}")
    label, results_dir = parts[0], parts[1]
    cp2_glob = parts[2] if len(parts) > 2 else ""
    puma_glob = parts[3] if len(parts) > 3 else ""
    return label, results_dir, cp2_glob, puma_glob
...
def main(args):
    ...
    specs = [_parse_model_spec(s) for s in args.models]
    MODEL_LABELS = [s[0] for s in specs]
    for i, label in enumerate(MODEL_LABELS):
        MODEL_COLORS[label] = MODEL_PALETTE[i % len(MODEL_PALETTE)]   # for the bar chart
    baseline_from = args.baseline_from or MODEL_LABELS[0]

    pg_models, uaa, baseline_df = [], {}, None
    for i, (label, results_dir, cp2_glob, puma_glob) in enumerate(specs):
        print(f"[load] {label}  <- {results_dir}")
        summary, bdf = load_results(results_dir)
        if label == baseline_from:
            baseline_df = bdf
        marker = MODEL_MARKERS[i % len(MODEL_MARKERS)]
        pg_models.append({"label": label, "summary": summary,
                          "color": MODEL_PALETTE[i % len(MODEL_PALETTE)],
                          "marker": marker, "size": 18 if marker == "*" else 11})
        cp2_dirs = _expand([cp2_glob]) if cp2_glob else []
        puma_dirs = _expand([puma_glob]) if puma_glob else []
        c = compute_uaa_per_iter(cp2_dirs, args.cp2_benchmark, args.total_num, f"{label}-CP2") if cp2_dirs else None
        p = compute_uaa_per_iter(puma_dirs, args.puma_benchmark, args.total_num, f"{label}-PUMA") if puma_dirs else None
        uaa[label] = (c, p)
    if baseline_df is None:
        baseline_df = load_results(specs[0][1])[1]

    # ProteinGym dot plot (baselines + N models, all on each assay's vertical line, error bars)
    plot_proteingym(pg_models, baseline_df, os.path.join(args.output_dir, "proteingym_comparison.svg"))

    # CP2/PUMA bar charts (baselines + N models) for all / ncaa / naa mutation subsets
    for subset_key, title, baselines_dict, fname in [
        ...
        for label in MODEL_LABELS:
            c, p = uaa[label]
            models_uaa.append((label,
                               c[subset_key] if c else (np.nan, np.nan),
                               p[subset_key] if p else (np.nan, np.nan)))
        _bar_chart(_build_barplot_data(models_uaa, baselines_dict), title,
                   os.path.join(args.output_dir, fname))

    print(f"\nAll figures saved to: {args.output_dir}")
    print("\n=== ProteinGym mean Spearman (per model) ===")
    for m in pg_models:
        vals = m['summary']['mean_spearman'].dropna()
        print(f"  {m['label']:32s} {vals.mean():.4f}  ({len(vals)} assays)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", dest="models", action="append", required=True,
        help='Repeatable: "label:results_dir:cp2_glob:puma_glob". cp2/puma globs optional. '
             'Add any new model by passing another --model — no code changes.')
    parser.add_argument("--baseline-from", default=None,
        help="model label whose results.csv supplies the ProteinGym per-assay baselines "
             "(default: first --model)")
    parser.add_argument("--cp2-benchmark", required=True)
    parser.add_argument("--puma-benchmark", required=True)
