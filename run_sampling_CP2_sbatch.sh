...
echo "============================================================================"
echo "UAAG CP2 NCAA Benchmark Sampling"
echo "============================================================================"
...
BENCHMARK_PATH=/flash/project_465002574/UAAG2_main/data/benchmarks/5ly1_cp2.pt
...
if [ $? -eq 0 ]; then
    echo "[$(date)] ✓ CP2 sampling completed successfully"
else
    echo "[$(date)] ✗ CP2 sampling failed with exit code $?"
    exit 1
fi

# Post-processing and analysis
SAMPLES_PATH="/scratch/project_465002574/ProteinGymSampling/run${RUN_ID}/Samples"
OUTPUT_DIR="/scratch/project_465002574/UNAAGI_result/results/${MODEL}/CP2_${MODEL}_variational_sampling_${NUM_SAMPLES}_split${SPLIT_INDEX}"

echo ""
echo "============================================================================"
echo "Starting CP2 Post-Processing and Analysis"
echo "============================================================================"
...
python scripts/result_eval_uniform_uaa.py \
    --benchmark /flash/project_465002574/UAAG2_main/data/uaa_benchmark_csv/CP2_reframe.csv \
    --aa_output ${SAMPLES_PATH}/aa_distribution.csv \
    --output_dir ${OUTPUT_DIR} \
    --total_num ${NUM_SAMPLES}

if [ $? -eq 0 ]; then
    echo "[$(date)] ✓ CP2 evaluation completed successfully"
else
    echo "[$(date)] ✗ CP2 evaluation failed with exit code $?"
    exit 1
fi

echo ""
echo "============================================================================"
echo "CP2 Analysis Completed Successfully!"
echo "============================================================================"
echo "End time: $(date)"
