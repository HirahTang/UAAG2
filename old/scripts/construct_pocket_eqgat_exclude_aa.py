...
class amino_acid_pocket:
    ...
    def get_neighbors(self, residue, exclude_aa):
        # residue: the input residue for reconstruction
        # self.residues: all other residues in the protein
...
def main(args):
    ...
    if not os.path.exists(f'data/benchmark/pdb_{args.exclude_aa}'):
        os.makedirs(f'data/benchmark/pdb_{args.exclude_aa}')
    ...
    for pdb_name in pdb_dir_current:
        ...
        for aa in tqdm(protein_object.residues):
            if aa['res_name'] in aa_dict.keys() and aa['res_name'] != 'GLY' and aa['res_name'] != args.exclude_aa:
                output_dict = {}
                output_dict['ligand'] = protein_object.get_atoms(aa)
                pocket_atoms = protein_object.get_neighbors(aa, args.exclude_aa)
                # from IPython import embed; embed()
                if not pocket_atoms:
                    # print("skip")
                    continue
                output_dict['pocket'] = pocket_atoms
                save_dict[f'{pdb_name}_'+aa['identity']] = output_dict
        # embed()
        with open(f'data/benchmark/pdb_{args.exclude_aa}/uaag_{pdb_name}.json', 'w') as json_file:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exclude_aa', type=str, help='Amino acids to exclude')
